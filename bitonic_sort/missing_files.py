...
for input_type in input_types:
    for size_power in input_sizes:
        ...
        for procs in mpi_procs:
            filename = os.path.join(cali_dir, "bitonic-mpi-p{}-a{}-i{}.cali".format(procs, size, input_type))
...
for input_type in input_types:
    for size_power in input_sizes:
        ...
        for threads in cuda_threads:
            filename = os.path.join(cali_dir, "bitonic-cuda-t{}-v{}-i{}.cali".format(threads, size, input_type))
            if is_missing_or_empty(filename):
